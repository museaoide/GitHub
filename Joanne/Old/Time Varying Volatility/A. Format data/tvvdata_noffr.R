...
tvvData_noffr<-function(data, countries, begin=c(1985,4), terminate=c(2008,1), credittype="apa", powerof=1){
...
for (i in 1:nCountries){
	...
	if (!is.null(terminate)){datay = window(datay, end=terminate)}
...
return(list(Y=Y,X=X, Tsigbrk=Tsigbrk, countries=Countrylst, dlength=dlength, Start=Startdates,End=Enddates))
}
