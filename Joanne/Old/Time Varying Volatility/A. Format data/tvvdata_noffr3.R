...
tvvData_noffr3<-function(data, commdata, countries, commIndexWeights =c(1/3, 1/3, 1/3) ,SecondaryBrk=c(0, 1984, 2006, 2010, 2014), begin=NULL, terminate=NULL, credittype="apa", powerof=1){
	##Function: stacks country data (log defl, log rgdp, short term rate, log credit2gdp, log real property prices, log commodity prices) to use in plutusTvv3..
	##and converts data into an annual time series that begins with year 1
		...
		##commdata: spreadsheet of commodity indices and the country exchange rates (LCU/USD). Should be a monthly time series. 
			#For Joanne:
			#commdata=read.csv("/Users/joanneim/commodities.csv")
			#commdata<-ts(commdata, start=c(1971,1), freq=12)

		##countries: vector of country abbreviation.
		##commIndexWeights =(0,0,0) on the indices (iENERGY ,iNONFUEL,iPRECIOUSMET)
...
#commodityIndex Data
	comm <- log(commIndexWeights[1]*commdata[,"iENERGY"]*commdata[,g] + commIndexWeights[2]* commdata[,"iNONFUEL"]*commdata[,g] + commIndexWeights[3]* commdata[,"iPRECIOUSMET"]*commdata[,g])
